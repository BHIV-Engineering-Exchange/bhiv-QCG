...
- `integration_harness.py`: The main controller that processes incoming contracts, calls live APIs for Replay/GC, invokes External runtime, runs consensus, and records evidence to the Evidence Ledger.
- `integration_interfaces.py`: Defines the translation layer from Python objects to standard HTTP requests targeting external (simulated via live endpoints) systems like External, GC, and Replay Authority.
...
- `external_runtime_server.py`: Dedicated live endpoint representing External Runtime for local simulation.
